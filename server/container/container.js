...
// The public api, specified on the application
// namespace should be considered the stable api.
function Container(parent) {
  ...
  this.factoryTypeInjections = dictionary(
    parent ? parent.factoryTypeInjections : null);
...
Container.prototype = {
  ...
  register: function (fullName, factory, options) {
    assert(validateFullName(fullName), 'fullName must be a proper full name');
    ...
    if (normalizedName in this.cache) {
      throw new Error('Cannot re-register: `' + fullName +
        '`, as it has already been looked up.');
    }
  ...
  makeToString: function (factory, fullName) {
    return factory.toString() || fullName;
  },
  ...
  typeInjection: function (type, property, fullName) {
    ...
    if (fullNameType === type) {
      throw new Error('Cannot inject a `' + fullName +
        '` on other ' + type + '(s). Register the `' +
        fullName + '` as a different type and perform the typeInjection.');
    }
  ...
  injection: function (fullName, property, injectionName) {
    ...
    if (this.cache[normalizedName]) {
      throw new Error('Attempted to register an injection ' +
        'for a type that has already been looked up. (\'' +
        normalizedName + '\', \'' + property + '\', \'' +
        injectionName + '\')');
    }
    addInjection(this.injections, normalizedName,
  ...
  factoryTypeInjection: function (type, property, fullName) {
    ...
    addTypeInjection(this.factoryTypeInjections,
      type, property, this.normalize(fullName));
  },
   ...
   * Injecting one fullName on a type
   ...
   // injecting one fullName on another fullName
   container.factoryInjection
   ('model:post', 'secondaryStore', 'store:secondary');
  ...
  factoryInjection: function (fullName, property, injectionName) {
    ...
    if (fullName.indexOf(':') === -1) {
      return this.factoryTypeInjection(normalizedName,
        property, normalizedInjectionName);
    }
    ...
    if (this.factoryCache[normalizedName]) {
      throw new Error('Attempted to register a factoryInjection ' +
        'for a type that has already ' +
        'been looked up. (\'' + normalizedName + '\', \'' +
        property + '\', \'' + injectionName + '\')');
    }

    addInjection(this.factoryInjections, normalizedName,
      property, normalizedInjectionName);
  },

  /**
   A depth first traversal, destroying the container,
   its descendant containers and all
...
function resolve(container, normalizedName) {
  ...
  var resolved = container.resolver(normalizedName) ||
    container.registry[normalizedName];
...
function illegalChildOperation(operation) {
  throw new Error(operation +
    ' is not currently supported on child containers');
}
...
function factoryFor(container, fullName) {
  ...
  if (!factory || typeof factory.extend !== 'function' ||
    (/*!Ember.MODEL_FACTORY_INJECTIONS &&*/ type === 'model')) {
...
function factoryInjectionsFor(container, fullName) {
  ...
  factoryInjections = factoryInjections.concat
  (container.factoryTypeInjections[type] || []);
  factoryInjections = factoryInjections.concat
  (container.factoryInjections[fullName] || []);
...
function instantiate(container, fullName) {
  ...
  if (factory) {
    ...
    if (typeof factory.extend === 'function') {
      ...
      // TODO: support new'ing for instantiation and
      // merge injections for pure JS Functions
...
function validateFullName(fullName) {
  if (!VALID_FULL_NAME_REGEXP.test(fullName)) {
    throw new TypeError('Invalid Fullname, expected: `type:name` got: ' +
      fullName);
  }
  return true;
}
